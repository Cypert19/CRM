...
export async function ensureUserRecord(): Promise<ActionResponse> {
  try {
    ...
    if (!existing) {
      const usersTable = admin.from("users") as ReturnType<typeof admin.from>;
...
export async function createWorkspaceForUser(
  ...
  try {
    ...
    // Create workspace
    const wsTable = admin.from("workspaces") as ReturnType<typeof admin.from>;
    const { data: workspace, error: wsError } = await wsTable
    ...
    const membersTable = admin.from("workspace_members") as ReturnType<typeof admin.from>;
    ...
    const pipelinesTable = admin.from("pipelines") as ReturnType<typeof admin.from>;
    const { data: pipeline, error: pipeError } = await pipelinesTable
    ...
    const stagesTable = admin.from("pipeline_stages") as ReturnType<typeof admin.from>;
    const { error: stagesError } = await stagesTable
